...
from plugins.Waifu4.cells.config import ConfigManager
from plugins.Waifu4.cells.generator import Generator
from plugins.Waifu4.cells.cards import Cards
from plugins.Waifu4.organs.memories import Memory
from plugins.Waifu4.systems.narrator import Narrator
from plugins.Waifu4.systems.value_game import ValueGame
from plugins.Waifu4.organs.thoughts import Thoughts
...
@register(name="Waifu4", description="Cuter than real waifu 333!", version="1.9.7", author="ElvisChenML")
class Waifu4(BasePlugin):
    def __init__(self, host: APIHost):
        self.ap = host.ap
        self._ensure_required_files_exist()
        self._generator = Generator(self.ap)
        self.waifu_cache: typing.Dict[str, WaifuCache] = {}
        self._set_permissions_recursively("data/plugins/Waifu4/", 0o777)

    async def initialize(self):
        await self._set_runner("waifu-mode")
        # 为新用户创建配置文件
        config_mgr = ConfigManager(f"data/plugins/Waifu4/config/waifu", "plugins/Waifu4/templates/waifu")
    ...
    async def _load_config(self, launcher_id: str, launcher_type: str):
        ...
        config_mgr = ConfigManager(f"data/plugins/Waifu4/config/waifu", "plugins/Waifu4/templates/waifu", launcher_id)
        await config_mgr.load_config(completion=True)
    ...
    def _ensure_required_files_exist(self):
        directories = ["data/plugins/Waifu4/cards", "data/plugins/Waifu4/config", "data/plugins/Waifu4/data"]
        ...
        for file in files:
            file_path = f"data/plugins/Waifu4/config/{file}"
            template_path = f"plugins/Waifu4/templates/{file}"
